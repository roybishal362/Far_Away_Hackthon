...
Headline: Grounding isn't a claim — we measured it: 51% vs 4% accuracy, and 0 vs 69 contradictions of official facts.
Body: Same questions, grounded vs a plain LLM, scored by an LLM judge (temperature 0) against 22 official SSW facts × 6 runs. Reproducible: script + gold set + per-fact verdicts committed (PROOF.md, eval/results.json). Grounded = 0 hallucinations in every run.
Layout: dark navy slide; full-slide imported 2-panel chart (left: accuracy 4% vs 51%; right: contradictions of official facts 69 vs 0; vermillion = grounded).
Image: [IMPORT: eval/ablation_chart.png] (do NOT AI-generate)
...
- eval/ablation_chart.png (auto-generated by `scripts/run_eval.py` from the REAL numbers: grounded 51% / 0 contradictions vs ungrounded 4% / 69)
- screenshots (real deployed app): home, SSE timeline mid-run, citation→ssw.go.jp composite, PDF dossier, mobile, HI/JA toggle
- qr codes: live URL (https://kakehashi-liard.vercel.app/), GitHub (github.com/roybishal362/Kakehashi), demo video — test from a phone
