...
const DEFAULTS: Profile = {
  ...
  sector_interest: "caregiving / nursing care",
  years_experience: 3,
  japanese_level: "none",
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-ink/60">
        The agents read official Japanese sources and live job data to build your plan.
      </p>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          ...
          <textarea
            ...
            placeholder="e.g. nursing, 3 years hospital experience"
          />
        </div>

        <div>
          <label className="label">Sector of interest</label>
          <select className="input" value={p.sector_interest} onChange={(e) => set("sector_interest", e.target.value)}>
            {SECTORS.map((s) => (
      ...
      <button type="submit" className="btn-primary mt-6 w-full" disabled={loading}>
        {loading ? (
          ...
          <>
            <Play className="h-4 w-4" /> Run the agents
          </>
        )}
      </button>
    </form>
